defmodule UserManager do
  def process_user(user) do
    ...
    if is_active && is_verified && !is_banned do
  ...
  def build_permissions(is_admin, has_profile) do
    base = [:read]
    with_write = if is_admin, do: [:write | base], else: base
    with_profile = if has_profile, do: [:edit_profile | with_write], else: with_write
    with_profile
  end
  ...
  def send_notification(user, message) do
    ...
    if has_notifications_enabled && is_verified && has_email do
  ...
  def update_status(user, new_status) do
    is_valid_status = new_status in [:active, :inactive, :suspended]
    has_status_changed = user.status != new_status
    is_locked = user.locked_at != nil

    if is_valid_status && has_status_changed && !is_locked do
