...
def _parse_json(raw: str, path: str) -> dict[str, Any]:
    try:
        ...
        # raise from preserves the JSON parse error in the exception chain
        raise ConfigError(
...
def _require_key(config: dict[str, Any], key: str) -> Any:
    try:
        ...
        raise MissingKeyError(
            f"Required configuration key {key!r} is missing"
        ) from exc
...
def load_from_env_or_file(env_var: str, fallback_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConfigError(
            f"Failed to load config (env {env_var}={path!r})"
        ) from exc
