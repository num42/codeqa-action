defmodule CodeQA.CombinedMetrics.SampleRunner do
  ...
  defp analyze(dir) do
    dir
    |> CodeQA.Collector.collect_files()
    |> CodeQA.Analyzer.analyze_codebase()
          ...
          |> Enum.flat_map(fn {behavior, behavior_data} ->
            ...
            if map_size(scalars) == 0 do
              ...
              {dot, norm_s_sq, norm_v_sq, contributions} =
                Enum.reduce(scalars, {0.0, 0.0, 0.0, []}, fn {{group, key}, scalar}, {d, ns, nv, contribs} ->
    ...
    |> Enum.reduce({%{}, %{updated: 0, deadzoned: 0, skipped: 0}}, fn
      {behavior, current_groups}, {acc_yaml, stats} ->
        ...
        case Map.get(report, report_key) do
          nil ->
            {Map.put(acc_yaml, behavior, current_groups),
          ...
          metrics ->
            ...
            {Map.put(acc_yaml, behavior, groups),
             %{stats | updated: stats.updated + n_updated, deadzoned: stats.deadzoned + n_deadzoned}}
        end
    end)
  end

  defp groups_from_report(metrics) do
    Enum.reduce(metrics, {%{}, 0.0, 0, 0}, fn {metric_key, data}, {groups, log_baseline, n_updated, n_deadzoned} ->
      [group, key] = String.split(metric_key, ".", parts: 2)

      if deadzone?(data.ratio) do
        {groups, log_baseline, n_updated, n_deadzoned + 1}
      else
        new_groups = Map.update(groups, group, %{key => data.suggested_scalar}, &Map.put(&1, key, data.suggested_scalar))
  ...
  defp format_yaml(data) do
    lines =
      ...
      |> Enum.flat_map(fn {behavior, groups} ->
        ...
        group_lines =
          groups
          |> Enum.filter(fn {k, v} -> k != "_log_baseline" and is_map(v) end)
        ...
        ["#{behavior}:" | baseline_line] ++ group_lines ++ [""]
      end)
