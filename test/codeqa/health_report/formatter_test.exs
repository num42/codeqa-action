defmodule CodeQA.HealthReport.FormatterTest do
  ...
  describe "plain formatter: cosine offenders with enriched data" do
    defp report_with_enriched_cosine_plain do
      %{
        metadata: %{
    ...
    test "cosine offenders table has 3 columns (File | Cosine | Details)" do
      result = Formatter.format_markdown(report_with_enriched_cosine_plain(), :default, :plain)
      assert result =~ "| File | Cosine | Details |"
      assert result =~ "|------|--------|---------|"
    end

    test "Details cell contains Why with direction indicators" do
      result = Formatter.format_markdown(report_with_enriched_cosine_plain(), :default, :plain)
      assert result =~ "**Why:** ↓ branching.mean_depth (−4.10), ↓ halstead.effort (−3.22)"
    end

    test "Details cell omits Why when top_metrics empty" do
      category = %{
        @enriched_cosine_category
        | behaviors: [
            %{
    ...
    test "Details cell contains Where" do
      result = Formatter.format_markdown(report_with_enriched_cosine_plain(), :default, :plain)
      assert result =~ "**Where:** line 89 (block), line 134 (block)"
    end

    test "Details cell omits Where when top_nodes empty" do
      category = %{
        @enriched_cosine_category
        | behaviors: [
            %{
    ...
    test "Details cell omits Where when top_nodes key absent" do
      category = %{
        @enriched_cosine_category
        | behaviors: [
            %{
              ...
              score: 42,
              grade: "D+",
              worst_offenders: [%{file: "lib/foo.ex", cosine: -0.5}]
            }
          ]
      }
    ...
    test "Fix in Details when hint present" do
      category = %{
        type: :cosine,
        key: "variable_naming",
    ...
    test "Fix omitted when hint absent" do
      category = %{
        type: :cosine,
        key: "nonexistent",
        name: "Nonexistent",
  ...
  describe "plain formatter: threshold offenders with enriched data" do
    defp report_with_enriched_threshold_plain do
      %{
        metadata: %{
          path: "/home/user/project",
          timestamp: "2026-03-11T00:00:00Z",
    ...
    test "threshold Issues cell contains Where when top_nodes present" do
      result =
        Formatter.format_markdown(report_with_enriched_threshold_plain(), :default, :plain)

      assert result =~ "**Where:** line 201 (block), line 312 (block)"
    end

    test "threshold Issues cell omits Where when top_nodes empty" do
      category = %{
        @enriched_threshold_category
        | worst_offenders: [
            %{
    ...
    test "threshold Issues cell omits Where when top_nodes key absent" do
      category = %{
        @enriched_threshold_category
        | worst_offenders: [
            %{
    ...
    test "Fix in threshold Issues when hint present" do
      result =
        Formatter.format_markdown(report_with_enriched_threshold_plain(), :default, :plain)
    ...
    test "Fix omitted from threshold Issues when hint absent" do
      category = %{
        @enriched_threshold_category
        | worst_offenders: [
            %{
